<x-layouts.admin>
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Statistik Perkawinan</h1>
        <a href="{{ route('kawin.create') }}" class="btn btn-primary text-white">Tambah Data</a>
    </div>
    ...
    <form id="bulk-delete-form" action="{{ route('kawin.bulk-destroy') }}" method="POST">
        @csrf
        @method('DELETE')
            ...
            <table class="table w-full">
                ...
                <tbody>
                    ...
                    <tr class="hover">
                        ...
                        <td class="font-bold">{{ $item->status }}</td>
                        <td>{{ number_format($item->jumlah, 0, ',', '.') }} Jiwa</td>
                        <td class="flex gap-2">
                            <a href="{{ route('kawin.edit', $item->id) }}" class="btn btn-sm btn-warning">Edit</a>
                            <form action="{{ route('kawin.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus data ini?');">
                                @csrf @method('DELETE')
