<x-layouts.admin>
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Statistik Pekerjaan</h1>
        <a href="{{ route('pekerjaan.create') }}" class="btn btn-primary text-white">Tambah Pekerjaan</a>
    </div>
    ...
    <form id="bulk-delete-form" action="{{ route('pekerjaan.bulk-destroy') }}" method="POST">
        @csrf
        @method('DELETE')
            ...
            <table class="table w-full table-zebra">
                ...
                <tbody>
                    ...
                    <tr class="hover">
                        ...
                        <td>{{ $index + 1 }}</td>
                        <td class="font-bold">{{ $item->nama_pekerjaan }}</td>
                        <td>{{ number_format($item->jumlah, 0, ',', '.') }} Jiwa</td>
                        <td class="flex gap-2">
                            <a href="{{ route('pekerjaan.edit', $item->id) }}" class="btn btn-sm btn-warning">Edit</a>
                            <form action="{{ route('pekerjaan.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus data ini?');">
                                @csrf @method('DELETE')
